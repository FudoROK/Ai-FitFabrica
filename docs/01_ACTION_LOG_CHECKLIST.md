...
- [x] Added a post-billing staging env template and stricter production env readiness checks: `.env.post-billing-staging.example` documents live-mode auth/billing/provider/Try-On settings, while `production_infrastructure_readiness_gate.py --require-production` now blocks placeholder secrets such as `replace-with-*` before deploy. Added runbook `docs/runbooks/post_billing_staging_env.md`, wired the template into `post_billing_acceptance_gate.py`, and included production infrastructure tests in `no_billing_acceptance_gate.py`. Verification passed: production/post-billing/no-billing gate checks, frontend build/lint/typecheck, and full backend acceptance `1205 passed, 0 warnings`.
