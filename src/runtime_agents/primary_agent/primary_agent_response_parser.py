...
from .primary_agent_response_mapper import build_parsed_payload_diagnostic
from .primary_agent_semantic_validator import validate_runtime_semantics
...
def _parse_strict_runtime_output(payload: dict[str, Any]) -> MainAgentRuntimeOutput:
    try:
        typed_output = MainAgentRuntimeOutput.model_validate(payload)
...
def _log_parsed_payload_diagnostic(payload: MainAgentRuntimeOutput) -> None:
    logger.info(
