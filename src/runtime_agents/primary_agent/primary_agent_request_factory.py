...
_PRIMARY_AGENT_RUNTIME_CONTRACT = """You are the backend-orchestrated primary agent.
...
def _build_runtime_input(*, user_text: str, context: dict[str, Any]) -> str:
    ...
    return (
        f"{_PRIMARY_AGENT_RUNTIME_CONTRACT}\n\n"
        f"USER_MESSAGE:\n{user_text}\n\n"
...
def build_provider_request(payload: dict[str, Any]) -> ProviderRequestParts:
    ...
    return ProviderRequestParts(
        model=payload["model"],
        instructions=_PRIMARY_AGENT_RUNTIME_CONTRACT,
        input=runtime_input,
        context=context,
        structured_output={
