"""Run Material / Texture live acceptance without executing the full Try-On workflow."""
...
REQUIRED_CASES = (
    "dark_or_blurry_fabric.png",
    "denim_texture.png",
    "knit_texture.png",
    "leather_like_finish.png",
    "matte_cotton_like.png",
    "no_material_evidence.png",
    "sheer_transparent_fabric.png",
    "shiny_satin_like.png",
)
...
class AcceptanceAsset:
    """One named Material / Texture acceptance asset."""
...
def collect_acceptance_assets(assets_dir: Path) -> list[AcceptanceAsset]:
    """Return the canonical 8-file Material / Texture acceptance dataset."""
    ...
    if missing:
        raise ValueError(f"missing required material texture acceptance files: {', '.join(missing)}")
    return [
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run Material / Texture live acceptance only.")
    parser.add_argument(
        "--assets-dir",
        type=Path,
        default=Path("test-assets/material-texture"),
        help="Directory containing the canonical 8-image Material / Texture dataset.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="JSONL output file. Defaults to output/material_texture_live_acceptance_<timestamp>.jsonl.",
    )
...
async def _run_acceptance(*, assets: list[AcceptanceAsset], settings) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Invoke only Material / Texture against the supplied assets."""

    providers = build_provider_runtime(settings)
    if providers.agent_runtime is None:
        raise RuntimeError("Material / Texture live acceptance requires llm.provider=vertex with agent_runtime enabled.")
    ...
    adapter = TryOnMaterialTextureAnalysisAdapter(
        ...
        preferred_model=getattr(settings, "try_on_material_texture_preferred_model", None),
    )
...
async def _run_one_asset(
    *,
    ...
    object_key = f"acceptance/material-texture/{asset.path.name}"
    ...
    try:
        analysis = await adapter.analyze(job_id=f"material-live-{asset.path.stem}", stored_inputs=[stored_input])
        actual_decision = "allowed"
        safe_code = None
        analysis_payload: dict[str, object] | None = analysis.model_dump(mode="json")
    except MaterialTextureAnalysisFailure as exc:
...
def _output_path(path: Path | None) -> Path:
    ...
    return Path("output") / f"material_texture_live_acceptance_{timestamp}.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    """Run the Material / Texture live acceptance CLI."""
    ...
    try:
        ...
        print("material_texture_live_acceptance_status=error")
        print(f"error={exc}")
        return 1

    print("material_texture_live_acceptance_status=completed")
