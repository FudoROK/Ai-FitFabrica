...
import type {
  ...
  AdminMarketplaceDiscoveryCandidate,
  BusinessProduct,
  SimilarSearchClickAnalyticsResponse,
} from "@/lib/api/business-catalog-contracts";
import { WebApiClient } from "@/lib/api/client";
import { getApiBaseUrl } from "@/lib/api/config";
import { DiscoveryCandidateReviewPanel } from "./business-catalog-review/discovery-candidate-review-panel";
import {
  getBusinessProductReviewReadiness,
  getBusinessProductReviewSummary,
  matchesBusinessProductReviewFilter,
} from "./business-catalog-review/model";
import {
  AdminOperationOrderPanel,
  BulkOperationDetailsPanel,
  ReviewQueueSummaryPanel,
  SimilarSearchAnalyticsPanel,
  StatusBadge,
  StatusPanel,
} from "./business-catalog-review/shared-panels";
import type {
  ...
  ReviewState,
  SearchIndexFilter,
} from "./business-catalog-review/types";
...
export function AdminBusinessCatalogReview() {
  ...
  const [discoveryCandidates, setDiscoveryCandidates] = useState<AdminMarketplaceDiscoveryCandidate[]>([]);
  ...
  const [candidateRejectReasons, setCandidateRejectReasons] = useState<Record<string, string>>({});
  const [categoryValidationInputs, setCategoryValidationInputs] = useState<Record<string, CategoryValidationInput>>({});
  const [discoveryCandidateFilters, setDiscoveryCandidateFilters] = useState<DiscoveryCandidateFilters>({
    category: "",
    city: "",
    sourceType: "all",
    status: "pending",
  ...
  async function loadProducts() {
    ...
    try {
      ...
      const [response, analytics, candidates] = await Promise.all([
        client.getAdminBusinessCatalogPendingProducts(credentials),
        client.getAdminSimilarSearchAnalytics(credentials),
        client.getAdminMarketplaceDiscoveryCandidates(credentials, {
          category: discoveryCandidateFilters.category,
          city: discoveryCandidateFilters.city,
          limit: 20,
          source_type:
            discoveryCandidateFilters.sourceType === "all" ? undefined : discoveryCandidateFilters.sourceType,
          status: discoveryCandidateFilters.status === "all" ? undefined : discoveryCandidateFilters.status,
        }),
      ]);
      setProducts(response.products);
      setSimilarSearchAnalytics(analytics);
      setDiscoveryCandidates(candidates.candidates);
      setState(response.products.length > 0 || candidates.candidates.length > 0 ? "ready" : "empty");
    } catch (requestError) {
      setProducts([]);
      setDiscoveryCandidates([]);
  ...
  async function approveDiscoveryCandidate(candidateId: string) {
    await mutateDiscoveryCandidate(candidateId, () =>
      apiClient().approveAdminMarketplaceDiscoveryCandidate(candidateId, credentials),
    );
  }

  async function rejectDiscoveryCandidate(candidateId: string) {
    const reason = candidateRejectReasons[candidateId]?.trim() ?? "";
    if (!reason) {
      setError("Reject reason is required for discovery candidates.");
      setState("error");
      return;
    }
    await mutateDiscoveryCandidate(candidateId, () =>
      apiClient().rejectAdminMarketplaceDiscoveryCandidate(candidateId, { reason }, credentials),
    );
  }

  async function archiveDiscoveryCandidate(candidateId: string) {
    await mutateDiscoveryCandidate(candidateId, () =>
      apiClient().archiveAdminMarketplaceDiscoveryCandidate(candidateId, credentials),
    );
  }

  async function mutateDiscoveryCandidate(
    candidateId: string,
    operation: () => Promise<{ candidate: AdminMarketplaceDiscoveryCandidate }>,
  ) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Discovery candidate action failed.");
      setState("error");
    } finally {
        ...
        {state === "ready" ? (
          <div className="grid gap-5">
            <DiscoveryCandidateReviewPanel
              ...
              filters={discoveryCandidateFilters}
              onArchive={(candidateId) => void archiveDiscoveryCandidate(candidateId)}
              onApprove={(candidateId) => void approveDiscoveryCandidate(candidateId)}
              onFiltersChange={setDiscoveryCandidateFilters}
              onReject={(candidateId) => void rejectDiscoveryCandidate(candidateId)}
              onReload={() => void loadProducts()}
              rejectReasons={candidateRejectReasons}
              setRejectReasons={setCandidateRejectReasons}
              submittingCandidateId={submittingCandidateId}
            />
