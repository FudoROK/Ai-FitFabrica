...
def test_content_package_runtime_dependencies_prefer_sql_and_portable_storage(monkeypatch) -> None:
    settings = _settings()
    monkeypatch.setattr(
        deps,
    ...
    runtime = deps.content_package_runtime_dependencies(settings)

    assert runtime.workflow_service is not None


def test_content_package_runtime_dependencies_are_cached(monkeypatch) -> None:
    settings = _settings()
    monkeypatch.setattr(
        deps,
        "portable_infrastructure",
    ...
    first = deps.content_package_runtime_dependencies(settings)
    second = deps.content_package_runtime_dependencies(settings)
