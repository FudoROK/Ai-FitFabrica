...
def _request(*, schema=None, task="primary_agent_reply_task") -> LLMRequest:
    return LLMRequest(
        task=task,
        input="hello",
...
def test_fake_provider_ok_text_and_json():
    provider = FakeProvider(task_outputs={"primary_agent_reply_task": {"text": "ok", "json": {"answer": "yes"}}})
...
def test_fake_provider_invalid_output_when_schema_mismatch():
    provider = FakeProvider(task_outputs={"primary_agent_reply_task": {"json": {"answer": 123}}})
    schema = {
        "type": "object",
        "required": ["answer"],
...
def test_fake_provider_rate_limited_failure():
    provider = FakeProvider(failing_tasks={"primary_agent_reply_task": "rate_limited"})
...
def test_fake_provider_timeout_failure():
    provider = FakeProvider(failing_tasks={"primary_agent_reply_task": "timeout"})
