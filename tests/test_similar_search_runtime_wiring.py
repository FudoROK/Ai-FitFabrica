...
def _settings() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        business_catalog_category_validation_mode="agent",
...
def test_similar_search_runtime_dependencies_use_qdrant_and_sql_catalog(monkeypatch) -> None:
    ...
    assert runtime.garment_identity_analyzer is not None


def test_business_catalog_sandbox_mode_does_not_replace_similar_search_analyzer(monkeypatch) -> None:
    settings = _settings()
    settings.business_catalog_category_validation_mode = "sandbox"
    monkeypatch.setattr(
        deps,
        "portable_infrastructure",
        lambda _settings: SimpleNamespace(sql_session_factory="session-factory", qdrant_client=object(), object_storage=object()),
    ...
    assert runtime.garment_identity_analyzer.__class__.__name__ == "GarmentIdentityAnalysisAdapter"
